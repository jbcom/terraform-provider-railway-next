...
conflict — and `uat-postgres` and `prod-postgres` can coexist in one tailnet.

The mechanism is `TS_SERVE_CONFIG`, which the official image already supports:

```json
{ "TCP": { "5432": { "TCPForward": "prod-postgres.railway.internal:5432" } } }
```
...
**Verified in production use**: an operator runs `psql prod-postgres` from a
...
resource "railway_tailscale" "postgres" {
  ...
  name     = "uat-postgres"          # the MagicDNS name an operator types
  ...
  upstream {
    host   = "uat-postgres.railway.internal"
    port   = 5432
    scheme = "tcp"                   # "tcp" | "http"
  }

  # Self-hosted control server. Omit for Tailscale's SaaS.
  login_server = "https://vpn.jonbogaty.com"
